...
class WorkflowTool(AgentBuilderBaseTool):
    ...
    async def _sync_workflows_to_version_config(self) -> None:
        try:
            ...
            await client.table('agent_versions').eq('version_id', current_version_id).update({'config': config})
    ...
    async def update_workflow(
        ...
        try:
            ...
            result = await client.table('agent_workflows').eq('id', workflow_id).update(update_data)
    ...
    async def delete_workflow(self, workflow_id: str) -> ToolResult:
        try:
            ...
            result = await client.table('agent_workflows').eq('id', workflow_id).delete()
    ...
    async def activate_workflow(self, workflow_id: str, active: bool = True) -> ToolResult:
        try:
            ...
            result = await client.table('agent_workflows').eq('id', workflow_id).update({'status': new_status})
