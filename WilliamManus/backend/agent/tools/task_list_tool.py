...
class TaskListTool(Tool):
    ...
    def _document_report_plan_redirect(self, task_texts: List[str]) -> Optional[str]:
        joined = "\n".join(str(text or "") for text in task_texts).lower()
        if not joined.strip():
            return None
        has_deliverable = bool(
            re.search(r"(report|analysis|markdown|html|deliverable|报告|分析)", joined)
        )
        has_exhaustive_extraction = bool(
            re.search(
                r"(all|every|each|full|exhaustive|全部|所有|每[个份页条道]|逐[个页条道])"
                r".{0,24}"
                r"(extract|transcribe|ocr|parse|read|content|question|item|row|page|file|pdf|document|"
                r"提取|抽取|转写|转录|读取|解析|内容|题目|页面|文件|文档)",
                joined,
            )
            or re.search(
                r"(extract|transcribe|ocr|parse|read|提取|抽取|转写|转录|读取|解析)"
                r".{0,24}"
                r"(all|every|each|full|exhaustive|全部|所有|每[个份页条道]|逐[个页条道])",
                joined,
            )
        )
        if not (has_deliverable and has_exhaustive_extraction):
            return None
        return (
            "DOCUMENT_REPORT_TASK_PLAN_REDIRECT: This task plan turns a report/"
            "analysis deliverable into exhaustive document extraction. Keep the "
            "plan coarse: collect sufficient evidence, write the requested "
            "deliverable(s), verify outputs. Do not create one task per page, file, "
            "row, item, or question unless the user explicitly asked for full "
            "transcription/extraction. Replace exhaustive extraction tasks with "
            "'collect enough evidence from the provided documents/data'. "
            '[Tool diagnostic: {"error_code": "DOCUMENT_REPORT_TASK_PLAN_REDIRECT"}]'
        )
            ...
            if result.data and result.data[0].get("content"):
                ...
                for i, t in enumerate(tasks_data):
                    try:
                        ...
                        tasks.append(task)
                        logger.debug(
                            f"Created task {i}: {task.id}, status: {repr(task.status)}"
                        )
                    except Exception as e:
    ...
    async def _save_data(self, sections: List[Section], tasks: List[Task]):
        ...
        try:
            ...
            print(
                f"🔍 _save_data: Processing {len(tasks)} tasks for serialization",
                file=sys.stderr,
            )
            ...
            for i, task in enumerate(tasks):
                ...
                print(
                    f"🔍 Task {i} - content type: {type(task.content)}", file=sys.stderr
                )
                print(
                    f"🔍 Task {i} - status type: {type(task.status)}, value: {repr(task.status)}",
                    file=sys.stderr,
                )
                print(
                    f"🔍 Task {i} - section_id type: {type(task.section_id)}",
                    file=sys.stderr,
                )
                ...
                if asyncio.iscoroutine(task.status):
                    print(
                        f"❌ FOUND COROUTINE in task.status for task {task.id}: {task.status}",
                        file=sys.stderr,
                    )
                    task.status = TaskStatus.PENDING
                ...
                if asyncio.iscoroutine(task.content):
                    print(
                        f"❌ FOUND COROUTINE in task.content for task {task.id}: {task.content}",
                        file=sys.stderr,
                    )
                ...
                if asyncio.iscoroutine(task.section_id):
                    print(
                        f"❌ FOUND COROUTINE in task.section_id for task {task.id}: {task.section_id}",
                        file=sys.stderr,
                    )
                ...
                try:
                    ...
                    print(
                        f"❌ model_dump FAILED for task {i}: {model_dump_error}",
                        file=sys.stderr,
                    )
                    ...
                    for field_name in ["id", "content", "status", "section_id"]:
                        try:
                            ...
                            print(
                                f"   {field_name}: {type(field_value)} = {repr(field_value)}",
                                file=sys.stderr,
                            )
                            if asyncio.iscoroutine(field_value):
                                print(
                                    f"   ❌ FIELD {field_name} IS COROUTINE!",
                                    file=sys.stderr,
                                )
                        except Exception as field_error:
                            print(
                                f"   ❌ Error accessing field {field_name}: {field_error}",
                                file=sys.stderr,
                            )

            # EMERGENCY DEBUG: Test content creation
            print(
                f"🔍 Creating content dict with {len(sections)} sections and {len(tasks)} tasks",
                file=sys.stderr,
            )
            try:
                sections_data = []
                ...
                for i, task in enumerate(tasks):
                    print(
                        f"🔍 Processing task {i} for content creation: {task.id}",
                        file=sys.stderr,
                    )
                    try:
                        ...
                        print(
                            f"✅ Task {i} model_dump success in content creation",
                            file=sys.stderr,
                        )
                    except Exception as task_dump_error:
                        print(
                            f"❌ Task {i} model_dump FAILED in content creation: {task_dump_error}",
                            file=sys.stderr,
                        )
                        raise

                content = {"sections": sections_data, "tasks": tasks_data}
            ...
            print(
                f"🔍 Database query completed: {len(result.data) if result.data else 0} records found",
                file=sys.stderr,
            )
            ...
            try:
                ...
                print(
                    f"JSON serialization successful, length: {len(json_content)}",
                    file=sys.stderr,
                )
            ...
            if result.data:
                ...
                print(
                    f"About to update with message_id: {message_id_for_update} (type: {type(message_id_for_update)})",
                    file=sys.stderr,
                )
                ...
                await client.table("messages").eq(
        ...
        try:
            ...
            if isinstance(sections, str):
                try:
                    ...
                    return ToolResult(
                        success=False,
                        output="Invalid 'sections': must be a JSON array or object",
                    )

                if isinstance(parsed_sections, dict):
                    normalized_sections = [parsed_sections]
                elif isinstance(parsed_sections, list):
                    normalized_sections = parsed_sections
                else:
                    return ToolResult(
                        success=False,
                        output="Invalid 'sections': must be a list of section objects",
                    )
                ...
                return ToolResult(
                    success=False, output="Invalid 'sections': unsupported type"
                )

            incoming_task_texts: List[str] = []
            if normalized_sections:
                for section_data in normalized_sections:
                    ...
                    if isinstance(task_list, str):
                        incoming_task_texts.append(task_list)
                    elif isinstance(task_list, (list, tuple)):
                        for task_item in task_list:
                            if isinstance(task_item, dict):
                                incoming_task_texts.append(
                                    str(task_item.get("content", ""))
                                )
                            else:
                                incoming_task_texts.append(str(task_item))
            elif task_contents:
                if isinstance(task_contents, str):
                    incoming_task_texts.append(task_contents)
                elif isinstance(task_contents, (list, tuple)):
                    incoming_task_texts.extend(str(item) for item in task_contents)

            redirect = self._document_report_plan_redirect(incoming_task_texts)
            if redirect:
                return ToolResult(success=False, output=redirect)

            if normalized_sections:
                # Batch creation across multiple sections
                for index, section_data in enumerate(normalized_sections):
                    if isinstance(section_data, str):
                        try:
                            section_data = json.loads(section_data)
                        except Exception:
                            return ToolResult(
                                success=False,
                                output=f"Invalid sections[{index}]: expected object with 'title' and 'tasks'",
                            )
                    ...
                    section_title_input = section_data.get("title")
                    task_list = section_data.get("tasks")

                    if (
                        not isinstance(section_title_input, str)
                        or not section_title_input.strip()
                    ):
                        return ToolResult(
                            success=False,
                            output=f"Invalid sections[{index}].title: must be a non-empty string",
                        )

                    if isinstance(task_list, str):
                        task_list = [task_list]
                    elif isinstance(task_list, tuple):
                        task_list = list(task_list)

                    if not isinstance(task_list, list):
                        return ToolResult(
                            success=False,
                            output=f"Invalid sections[{index}].tasks: must be an array of task strings",
                        )

                    normalized_task_list: List[str] = []
                    for task_item in task_list:
                        if isinstance(task_item, str):
                            task_content = task_item.strip()
                        elif isinstance(task_item, dict):
                            task_content = str(task_item.get("content", "")).strip()
                        else:
                            task_content = str(task_item).strip()

                        if task_content:
                            normalized_task_list.append(task_content)

                    if not normalized_task_list:
                        return ToolResult(
                            success=False,
                            output=f"Invalid sections[{index}].tasks: at least one non-empty task is required",
                        )

                    # Find or create section
                    title_lower = section_title_input.strip().lower()
                    if title_lower in title_map:
                        target_section = title_map[title_lower]
                    else:
                        target_section = Section(title=section_title_input.strip())
                        existing_sections.append(target_section)
                        title_map[title_lower] = target_section
                        created_sections += 1

                    # Create tasks in this section
                    for task_content in normalized_task_list:
                        new_task = Task(
                ...
                if not task_contents:
                    return ToolResult(
                        success=False,
                        output="必须提供 'sections' 数组或 'task_contents' 与 section 信息",
                    )

                if isinstance(task_contents, str):
                    normalized_task_contents = [task_contents.strip()]
                elif isinstance(task_contents, tuple):
                    normalized_task_contents = [
                        str(item).strip() for item in task_contents
                    ]
                elif isinstance(task_contents, list):
                    normalized_task_contents = [
                        str(item).strip() for item in task_contents
                    ]
                else:
                    return ToolResult(
                        success=False, output="'task_contents' 必须是字符串或字符串数组"
                    )

                normalized_task_contents = [
                    item for item in normalized_task_contents if item
                ]
                if not normalized_task_contents:
                    return ToolResult(success=False, output="'task_contents' 不能为空")
                ...
                # Create tasks
                for content in normalized_task_contents:
    ...
    async def update_tasks(
        ...
        try:
            # 标准化 task_ids 总是一个列表
            if task_ids is None:
                return ToolResult(success=False, output="task_ids is required")

            if isinstance(task_ids, (list, tuple, set)):
                target_task_ids = [str(task_id).strip() for task_id in task_ids]
            else:
                target_task_ids = [str(task_ids).strip()]

            target_task_ids = [task_id for task_id in target_task_ids if task_id]
            if not target_task_ids:
                return ToolResult(
                    success=False,
                    output="task_ids must include at least one valid task id",
                )
            ...
            if status is not None:
                if asyncio.iscoroutine(status):
                    return ToolResult(
                        success=False,
                        output="Invalid status: coroutine is not supported",
                    )
                ...
                status_str = status_alias.get(
                    str(status).strip().lower(), str(status).strip().lower()
                )

                try:
                    normalized_status = TaskStatus(status_str)
                except ValueError:
                    valid_statuses = [status_item.value for status_item in TaskStatus]
                    return ToolResult(
                        success=False,
                        output=f"Invalid status '{status}'. Valid values: {valid_statuses}",
                    )
            ...
            for tid in target_task_ids:
                try:
                    ...
                    if normalized_status is not None:
                        ...
                        logger.debug(
                            f"Updating status for task {tid}, status type: {type(normalized_status)}, value: {repr(normalized_status)}"
                        )
                        try:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Pydantic validation failed for task {tid}: {validation_error}"
                                )
                                task.status = TaskStatus.PENDING  # Safe fallback

                            logger.debug(
                                f"Successfully updated status for task {tid} to {repr(task.status)}"
                            )
                        except Exception as status_error:
                            logger.error(
                                f"Error updating status for task {tid}: {status_error}"
                            )
                            raise
    ...
    async def delete_tasks(
        ...
        try:
            ...
            if not task_ids and not section_ids:
                return ToolResult(
                    success=False, output="Must provide either task_ids or section_ids"
                )
