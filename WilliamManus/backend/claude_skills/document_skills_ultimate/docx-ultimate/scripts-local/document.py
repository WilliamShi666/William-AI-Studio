...
Usage:
    ...
    # Add footnotes
    doc.add_footnote(anchor_node=node, text="Footnote body text.")
...
class Document:
    ...
    def __init__(
        ...
        # Footnote file path
        self.footnotes_path = self.word_path / "footnotes.xml"
        ...
        # Load existing footnotes and determine next ID
        self.next_footnote_id = self._get_next_footnote_id()
    ...
    def add_footnote(self, anchor_node, text: str) -> int:
        """
        Add a footnote anchored to a specific run or paragraph node.

        Inserts a superscript reference mark after anchor_node in document.xml,
        and appends the footnote body to footnotes.xml.

        Args:
            anchor_node: DOM element (w:r or w:p) where the reference mark is inserted.
                         For w:p, the reference run is appended inside it.
                         For w:r (or any other element), it is inserted after it.
            text: Footnote body text.

        Returns:
            The footnote ID created (integer >= 1).

        Example:
            node = doc["word/document.xml"].get_node(tag="w:r", contains="cited claim")
            doc.add_footnote(node, "Source: Smith et al., 2024.")
        """
        footnote_id = self.next_footnote_id
        ref_xml = self._footnote_ref_run_xml(footnote_id)
        if anchor_node.tagName == "w:p":
            self._document.append_to(anchor_node, ref_xml)
        else:
            self._document.insert_after(anchor_node, ref_xml)
        self._add_to_footnotes_xml(footnote_id, text)
    ...
    def save(self, destination=None, validate=True) -> None:
        ...
        # Only ensure footnote relationships and content types if footnotes file exists
        if self.footnotes_path.exists():
            self._ensure_footnote_relationships()
            self._ensure_footnote_content_types()
    ...
    def _get_next_footnote_id(self):
        """Get next available footnote ID (IDs -1 and 0 are reserved separators; user IDs start at 1)."""
        if not self.footnotes_path.exists():
            return 1
        editor = self["word/footnotes.xml"]
        max_id = 0
        for fn_elem in editor.dom.getElementsByTagName("w:footnote"):
            fn_id = fn_elem.getAttribute("w:id")
            if fn_id:
                try:
                    max_id = max(max_id, int(fn_id))
                except ValueError:
                    pass
  ...
  <w:p w14:paraId="{para_id}" w14:textId="77777777">
    ...
    def _add_to_footnotes_xml(self, footnote_id: int, text: str):
        """Append a footnote entry to footnotes.xml, creating from template if needed."""
        if not self.footnotes_path.exists():
            shutil.copy(TEMPLATE_DIR / "footnotes.xml", self.footnotes_path)
        editor = self["word/footnotes.xml"]
        root = editor.get_node(tag="w:footnotes")
        escaped_text = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        footnote_xml = f'''<w:footnote w:type="normal" w:id="{footnote_id}">
  <w:p>
    <w:pPr><w:pStyle w:val="FootnoteText"/></w:pPr>
    <w:r><w:rPr><w:rStyle w:val="FootnoteReference"/></w:rPr><w:footnoteRef/></w:r>
    <w:r><w:t xml:space="preserve"> {escaped_text}</w:t></w:r>
  </w:p>
</w:footnote>'''
        editor.append_to(root, footnote_xml)
...
<w:r>
  ...
  <w:commentReference w:id="{comment_id}"/>
    ...
    def _footnote_ref_run_xml(self, footnote_id: int) -> str:
        """Generate inline reference run XML for document.xml."""
        return f'''<w:r>
  ...
  <w15:presenceInfo w15:providerId="None" w15:userId="{escaped_author}"/>
    ...
    def _ensure_comment_relationships(self):
        ...
        missing = [
            (rel_type, target)
            for rel_type, target in all_rels
        ...
        for i, (rel_type, target) in enumerate(missing):
            rel_xml = f'<{prefix}Relationship Id="rId{next_rid_num + i}" Type="{rel_type}" Target="{target}"/>'
            editor.append_to(root, rel_xml)
    ...
    def _ensure_footnote_relationships(self):
        """Ensure word/_rels/document.xml.rels has the footnotes relationship."""
        editor = self["word/_rels/document.xml.rels"]
        if self._has_relationship(editor, "footnotes.xml"):
            return
        root = editor.dom.documentElement
    ...
    def _ensure_footnote_content_types(self):
        """Ensure [Content_Types].xml has the footnotes content type."""
        editor = self["[Content_Types].xml"]
        if self._has_override(editor, "/word/footnotes.xml"):
            return
        root = editor.dom.documentElement
        editor.append_to(
            root,
