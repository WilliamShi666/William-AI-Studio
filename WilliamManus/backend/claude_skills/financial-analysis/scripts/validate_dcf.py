...
class DCFModelValidator:
    ...
    def validate_all(self, ticker: Optional[str] = None) -> dict:
        """
        ...
        if ticker:
            self.check_market_sanity(ticker)
    ...
    def check_market_sanity(self, ticker: str):
        """Compare model's implied share price against current market price."""
        try:
            import yfinance as yf
        except ImportError:
            self.warnings.append("Market sanity check skipped: yfinance not installed (pip install yfinance)")
            return
        ...
        try:
            ...
            if cached and cached.get("price"):
                market_price = cached["price"]
            else:
                import yfinance as yf
                market_price = yf.Ticker(ticker).info.get('currentPrice')
        except Exception as e:
            self.warnings.append(f"Market sanity check skipped: could not fetch market price ({e})")
            return

        if market_price is None:
            self.warnings.append(f"Market sanity check skipped: no currentPrice returned for {ticker}")
            return

        # Scan DCF sheet for "Implied Share Price" label and adjacent numeric value
        model_price = None
        try:
            dcf_sheet = self.workbook_values['DCF']
            for row in dcf_sheet.iter_rows():
                for cell in row:
                    if cell.value and isinstance(cell.value, str) and 'implied share price' in cell.value.lower():
                        for offset in range(1, 6):
                            adjacent = dcf_sheet.cell(cell.row, cell.column + offset).value
                            if isinstance(adjacent, (int, float)) and adjacent > 0:
                                model_price = adjacent
                                break
            ...
            self.warnings.append("Market sanity check skipped: DCF sheet not found")
            return
        except Exception as e:
            self.warnings.append(f"Market sanity check skipped: error reading DCF sheet ({e})")
            return

        if model_price is None:
            self.warnings.append("Market sanity check skipped: could not locate 'Implied Share Price' in DCF sheet")
            return

        diff_pct = abs(model_price - market_price) / market_price * 100
        if diff_pct > 50:
            self.warnings.append(
                f"Market sanity: model price ${model_price:.2f} vs market price ${market_price:.2f} (diff {diff_pct:.1f}%)"
            )
        else:
            self.info.append(
                f"✓ Market sanity: model price ${model_price:.2f} vs market price ${market_price:.2f}"
            )


def validate_dcf_model(excel_path: str, ticker: Optional[str] = None) -> dict:
    """
    Validate a DCF model Excel file

    Args:
        excel_path: Path to Excel DCF model
        ticker: Optional ticker symbol for market sanity check

    Returns:
        Dict with validation results
    """
    validator = DCFModelValidator(excel_path)
    return validator.validate_all(ticker=ticker)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate a DCF model Excel file"
    )
    parser.add_argument("excel_file", help="Path to Excel DCF model")
    parser.add_argument("output_file", nargs="?", help="Optional output JSON file")
    parser.add_argument("--ticker", help="Ticker symbol for market sanity check (e.g. AAPL)")
    args = parser.parse_args()

    if not args.excel_file:
        print("Usage: python validate_dcf.py <excel_file> [output.json] [--ticker TICKER]")
        ...
        print("Example: python validate_dcf.py model.xlsx --ticker AAPL")
        sys.exit(1)

    try:
        results = validate_dcf_model(args.excel_file, ticker=args.ticker)
