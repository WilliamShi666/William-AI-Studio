...
def _is_qwen35_model(name: Optional[str]) -> bool:
    lowered = str(name or "").lower()
    return "qwen3.5-plus" in lowered or "qwen3.5-397b-a17b" in lowered


def _normalize_model_alias_for_display(alias: str, full_name: str) -> str:
    if _is_qwen35_model(alias) or _is_qwen35_model(full_name):
    ...
    try:
        ...
        if config.ENV_MODE == EnvMode.LOCAL:
            ...
            for short_name, full_name in MODEL_NAME_ALIASES.items():
                display_alias = _normalize_model_alias_for_display(short_name, full_name)
                dedupe_key = (full_name, display_alias)
        ...
        for short_name, full_name in MODEL_NAME_ALIASES.items():
            ...
            normalized_short_name = _normalize_model_alias_for_display(
                short_name,
                full_name,
            )
            ...
            if short_name != full_name and not short_name.startswith("openai/") and not short_name.startswith("anthropic/") and not short_name.startswith("openrouter/") and not short_name.startswith("xai/"):
                if full_name not in model_aliases:
                    model_aliases[full_name] = normalized_short_name
