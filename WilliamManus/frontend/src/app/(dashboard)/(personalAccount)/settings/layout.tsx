...
export default function PersonalAccountSettingsPage({
  ...
  const items = [
    ...
    ...(billingUiEnabled ? [{ name: t('menu.billing'), href: '/settings/billing' }] : []),
    ...(billingUiEnabled ? [{ name: t('menu.usageLogs'), href: '/settings/usage-logs' }] : []),
    ...(isLocalMode() ? [{ name: t('menu.envManager'), href: '/settings/env-manager' }] : []),
  ];
  return (
    <>
