...
export async function POST(
  ...
  try {
    ...
    const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || process.env.BACKEND_URL || 'http://localhost:8002/api';
    const targetUrl = `${backendUrl}/webhooks/trigger/${workflowId}`;
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/webhooks/test/${workflowId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
