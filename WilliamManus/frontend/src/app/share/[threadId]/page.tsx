...
export default function ThreadPage({
  ...
  const [agentStatus, setAgentStatus] = useState<
    'idle' | 'running' | 'connecting' | 'disconnecting' | 'error'
  >('idle');
  ...
  const handleStreamingToolCall = useCallback(
    (toolCall: StreamingToolCall | null) => {
      ...
      const isWriteFileTool = toolName === 'write-file';
      ...
      const normalizedWriteFileArgs = isWriteFileTool
        ? normalizeWriteFileArgs(toolCall.arguments)
        : null;
      if (normalizedWriteFileArgs) {
        toolArguments =
          stringifyNormalizedWriteFileArgs(normalizedWriteFileArgs) ||
          toolArguments;
      }
      ...
      if (
        ...
        if (matchingTag) {
          if (normalizedWriteFileArgs && isWriteFileTool) {
            formattedContent =
              stringifyNormalizedWriteFileArgs(normalizedWriteFileArgs) ||
              toolArguments;
          } else {
      ...
      setToolCalls((prev) => {
        ...
        if (existingStreamingIndex !== -1) {
          ...
          if (isWriteFileTool) {
            const previousArgs = normalizeWriteFileArgs(
              prev[existingStreamingIndex].assistantCall.content,
            );
            const nextArgs = normalizeWriteFileArgs(formattedContent);
            const previousBody = previousArgs?.file_contents || '';
            const nextBody = nextArgs?.file_contents || '';
            if (
              previousArgs &&
              (!nextArgs || previousBody.length > nextBody.length)
            ) {
              nextContent =
                stringifyNormalizedWriteFileArgs(previousArgs) ||
                prev[existingStreamingIndex].assistantCall.content ||
                formattedContent;
            }
          }
    ...
    async function loadData() {
      ...
      try {
        ...
        if (isMounted) {
          if (projectData) {
            const rawSandboxId =
              typeof projectData.sandbox === 'string'
                ? projectData.sandbox
                : projectData.sandbox?.id || null;
            const normalizedProject: Project = {
              ...projectData,
              file_delivery_source: resolveProjectFileDeliverySource(
                projectData,
                rawSandboxId,
              ),
            };
