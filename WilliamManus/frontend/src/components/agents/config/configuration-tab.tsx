  ...
  const handleSystemPromptChange = (value: string) => {
    if (!isSystemPromptEditable && isSunaAgent) {
      toast.error("System prompt cannot be edited", {
        description: "Roys Alpha's system prompt is managed centrally and cannot be changed.",
      });
      return;
    }
  ...
  const handleToolsChange = (tools: Record<string, boolean | { enabled: boolean; description: string }>) => {
    if (!areToolsEditable && isSunaAgent) {
      toast.error("Tools cannot be modified", {
        description: "Roys Alpha's default tools are managed centrally and cannot be changed.",
      });
      return;
    }
          ...
          {isSunaAgent && (
            <div className="p-4 bg-primary/10 border border-primary-200 rounded-xl">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-primary-800">默认代理</span>
              </div>
              <p className="text-sm text-primary-700">
                这是系统默认代理，系统提示词和工具由系统集中管理。
                你可以自定义集成、知识库、剧本和触发器来个性化体验。
              </p>
            </div>
          )}
