...
export function AgentModelSelector({
  ...
  const normalizeModelId = (modelId?: string): string => {
    ...
    if (modelsData?.models) {
      ...
      if (exactMatch) return toCanonicalModelId(exactMatch.short_name || exactMatch.id);

      const fullMatch = modelsData.models.find(m => m.id === modelId);
      if (fullMatch) return toCanonicalModelId(fullMatch.short_name || fullMatch.id);
      
      if (modelId.startsWith('openrouter/')) {
        const shortName = modelId.replace('openrouter/', '');
        const shortMatch = modelsData.models.find(m => m.short_name === shortName);
        if (shortMatch) return toCanonicalModelId(shortMatch.short_name || shortMatch.id);
      }
    }
  ...
  const handleSelect = (modelId: string) => {
    ...
    if (isLocalMode() || canAccessModel(modelId)) {
      ...
      toast.error(t('chat.modelUnavailable'));
    }
  };
  ...
  const renderModelOption = (model: any, index: number) => {
    ...
    const isRecommended = Boolean(model.recommended);
