...
import { config, isBillingUiEnabled, isLocalMode, isStagingMode } from '@/lib/config';
...
export function DashboardContent() {
  ...
  const selectedAgent = selectedAgentId
    ...
    : null;
  const displayName = selectedAgent?.name === 'Suna' ? t('agent.default') : (selectedAgent?.name || 'Roys Alpha');
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (options?.shadow_clone_mode) {
        formData.append('shadow_clone_mode', options.shadow_clone_mode);
      }
      if (options?.shadow_clone_main_model) {
        formData.append(
          'shadow_clone_main_model',
          options.shadow_clone_main_model,
        );
      }
      if (options?.shadow_clone_subagent_model) {
        formData.append(
          'shadow_clone_subagent_model',
          options.shadow_clone_subagent_model,
        );
      }
