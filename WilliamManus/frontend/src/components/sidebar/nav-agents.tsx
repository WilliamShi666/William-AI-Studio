...
export function NavAgents() {
  ...
  const isInThreadView = pathname?.includes('/thread/') ?? false;
  ...
  // Prevent lingering bulk-selection state while a thread is open.
  useEffect(() => {
    if (isInThreadView && selectedThreads.size > 0) {
      setSelectedThreads(new Set());
    }
  }, [isInThreadView, selectedThreads.size]);
  ...
  const handleMultiDelete = () => {
    ...
    setThreadToDelete({
      ...
      name: selectedThreads.size > 3
        ? (isZh ? `${selectedThreads.size} 个会话` : `${selectedThreads.size} conversations`)
        : threadNames
    });
  ...
  const confirmDelete = async () => {
    ...
    if (threadToDelete.id !== "multiple") {
          ...
          deleteThreadMutation(
            { threadId, sandboxId },
            {
              onSuccess: () => {
                ...
                toast.success(isZh ? '对话已删除' : 'Conversation deleted successfully');
              },
              onSettled: () => {
      ...
      // Show initial toast
      toast.info(isZh
        ? `正在删除 ${threadIdsToDelete.length} 个会话...`
        : `Deleting ${threadIdsToDelete.length} conversations...`);
      ...
      try {
        ...
        deleteMultipleThreadsMutation(
          ...
          },
          {
            ...
            onError: (error) => {
              ...
              toast.error(isZh ? '删除会话时出错' : 'Error deleting conversations');
            },
            onSettled: () => {
              setThreadToDelete(null);
              isPerformingActionRef.current = false;
        ...
        toast.error(isZh ? '启动删除流程时出错' : 'Error initiating deletion process');
  ...
  return (
    <SidebarGroup>
      <div className="flex justify-between items-center">
        <SidebarGroupLabel>{isZh ? '任务' : 'Tasks'}</SidebarGroupLabel>
            ...
            {isLoading ? (
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              ...
                              >
                                ...
                                <span className="sr-only">{isZh ? '更多操作' : 'More actions'}</span>
                              </button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent
                            ...
                            >
                              <DropdownMenuItem onClick={() => {
                                ...
                                <span>{isZh ? '分享对话' : 'Share chat'}</span>
                              </DropdownMenuItem>
                              <DropdownMenuItem asChild>
                                ...
                                >
                                  ...
                                  <span>{isZh ? '在新标签页打开' : 'Open in new tab'}</span>
                                </a>
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
              ...
              <SidebarMenuItem>
                <SidebarMenuButton className="text-sidebar-foreground/70">
                  <span>{isZh ? '暂无任务' : 'No tasks yet'}</span>
                </SidebarMenuButton>
              </SidebarMenuItem>
            )}
          </>
      ...
      {(isDeletingSingle || isDeletingMultiple) && totalToDelete > 0 && (
        <div className="mt-2 px-2">
          <div className="text-xs text-muted-foreground mb-1">
            {isZh
              ? `正在删除 ${deleteProgress > 0 ? `(${Math.floor(deleteProgress)}%)` : '...'}`
