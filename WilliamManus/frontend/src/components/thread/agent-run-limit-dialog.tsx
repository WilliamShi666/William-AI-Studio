...
const RunningThreadItem: React.FC<RunningThreadItemProps> = ({
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      toast.success(isZh ? '已停止代理运行' : 'Agent stopped successfully');
      onThreadStopped();
    } catch (error) {
      console.error('Failed to stop agent:', error);
      toast.error(isZh ? '停止代理失败' : 'Failed to stop agent');
    }
  };
  ...
  const getThreadDisplayName = () => {
    ...
    return isZh
      ? `会话 ${threadInfo.threadId.slice(0, 8)}...`
      : `Thread ${threadInfo.threadId.slice(0, 8)}...`;
  };
  ...
  const getProjectDisplayName = () => {
    ...
    if (!threadInfo.projectId) {
      return isZh ? '无项目' : 'No Project';
    }
    return isZh
      ? `项目 ${threadInfo.projectId.slice(0, 8)}...`
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                ...
                <span className="sr-only">{isZh ? '停止代理' : 'Stop agent'}</span>
              </Button>
            </TooltipTrigger>
            <TooltipContent>{isZh ? '停止该代理' : 'Stop this agent'}</TooltipContent>
          </Tooltip>
        )}
            ...
            <TooltipTrigger asChild>
              ...
              >
                <Link href={`/projects/${threadInfo.projectId}/thread/${threadInfo.threadId}`} target="_blank">
                  ...
                  <span className="sr-only">{isZh ? '打开会话' : 'Open thread'}</span>
                </Link>
              </Button>
            </TooltipTrigger>
            <TooltipContent>{isZh ? '打开会话' : 'Open thread'}</TooltipContent>
          </Tooltip>
        )}
      </div>
    </div>
...
export const AgentRunLimitDialog: React.FC<AgentRunLimitDialogProps> = ({
  ...
  return (
    <UpgradeDialog
      ...
      icon={AlertTriangle}
      title={isZh ? '并行运行已达上限' : 'Parallel runs limit reached'}
      description={isZh
        ? '当前并行运行数量已达到上限。'
    ...
    >
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-medium">
              {isZh ? '当前运行中的代理' : 'Currently running agents'}
            </h4>
          </div>
          ...
          {isLoadingThreads ? (
            ...
            <div className="text-center py-4 text-sm text-muted-foreground">
              <p>
                {isZh
                  ? `检测到 ${runningCount} 个运行中的代理，但无法加载会话详情。`
                  : `Found ${runningCount} running agents but unable to load thread details.`}
              </p>
              <p className="text-xs mt-1">
                {isZh ? '会话 ID' : 'Thread IDs'}: {JSON.stringify(runningThreadIds)}
              </p>
            </div>
          ) : (
              ...
              {runningThreadsInfo.length > 5 && (
                <div className="text-center py-2">
                  <Badge variant="outline" className="text-xs">
                    {isZh
                      ? `还有 ${runningThreadsInfo.length - 5} 个运行中`
      ...
      <div className="space-y-3">
        <h4 className="text-sm font-medium">
          {isZh ? '你可以这样做' : 'What can you do?'}
        </h4>
        <ul className="space-y-2 text-sm text-muted-foreground">
          <li className="flex items-start gap-2">
            <div className="h-1.5 w-1.5 rounded-full bg-muted-foreground mt-2 flex-shrink-0" />
            <span>
              {isZh ? '点击停止按钮结束正在运行的代理' : (
                <>
                  Click the <Square className="h-3 w-3 inline mx-1" /> button to stop running agents
                </>
              )}
            </span>
          </li>
          <li className="flex items-start gap-2">
            <div className="h-1.5 w-1.5 rounded-full bg-muted-foreground mt-2 flex-shrink-0" />
            <span>
              {isZh ? '等待代理自动完成任务' : 'Wait for an agent to complete automatically'}
            </span>
          </li>
        </ul>
      </div>
