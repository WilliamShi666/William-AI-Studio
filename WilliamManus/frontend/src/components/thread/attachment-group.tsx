...
export function AttachmentGroup({
    ...
    ): UploadedFile['status'] | undefined => {
        if (typeof file === 'string') return undefined;
        return file.status;
    };

    const getFileErrorMessage = (file: string | UploadedFile): string | undefined => {
        if (typeof file === 'string') return undefined;
                    ...
                    {sortedFilesWithMeta.map((item, index) => (
                        ...
                        >
                            <FileAttachment
                                ...
                                status={getFileStatus(item.file)}
                                errorMessage={getFileErrorMessage(item.file)}
                                className={cn(
