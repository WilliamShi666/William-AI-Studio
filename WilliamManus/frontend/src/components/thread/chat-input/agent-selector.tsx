...
export const AgentSelector: React.FC<AgentSelectorProps> = ({
  ...
  const resolveAgentName = (name?: string) => {
    if (!name || name === 'Suna') {
  ...
  const filteredAgents = allAgents.filter((agent) => {
    const displayName = resolveAgentName(agent.name);
    return (
  ...
  const getAgentDisplay = () => {
    ...
    if (selectedAgent) {
      ...
      return {
        name: resolveAgentName(selectedAgent.name),
    ...
    return {
      name: resolveAgentName(defaultAgent?.name) || 'Roys Alpha',
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-muted-foreground/20 scrollbar-track-transparent px-1.5">
            {agentsLoading ? (
              <div className="px-4 py-6 text-sm text-muted-foreground/70 text-center">
                <div className="animate-pulse">{t('agent.loadingAgents')}</div>
              </div>
            ) : sortedFilteredAgents.length === 0 ? (
              <div className="px-4 py-6 text-sm text-muted-foreground/70 text-center">
                <Search className="h-6 w-6 mx-auto mb-2 opacity-40" />
                <p>{t('agent.noAgentsFound')}</p>
                <p className="text-xs mt-1 opacity-60">{t('agent.tryAdjustSearch')}</p>
              </div>
            ) : (
