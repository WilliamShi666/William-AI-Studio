...
export function SiteHeader({
  ...
  const viewFilesLabel = isZh ? '查看任务文件' : 'View files in task';
  const shareChatLabel = isZh ? '分享对话' : 'Share chat';
  const toggleComputerLabel = isZh ? '切换电脑预览 (CMD+I)' : 'Toggle computer preview (CMD+I)';
  const renameTooltip = isZh ? '点击重命名项目' : 'Click to rename project';
  const renameMissingId = isZh ? '无法重命名：缺少项目 ID' : 'Cannot rename: Project ID is missing';
  const renameFailed = isZh ? '重命名项目失败' : 'Failed to rename project';
  const updateFailed = isZh ? '更新项目失败' : 'Failed to update project';
  const debugLabel = isZh ? '调试' : 'Debug';
  ...
  const saveNewName = async () => {
    ...
    if (editName !== projectName) {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : renameFailed;
  ...
  return (
    <>
      <header className={cn(
        "bg-background/80 backdrop-blur-md border-b border-border/40 sticky top-0 flex h-14 shrink-0 items-center gap-2 z-20 w-full transition-all duration-200",
        isMobile && "px-2"
      )}>
