...
const extractFromNewFormat = (content: any): { 
  ...
  if ('tool_execution' in parsedContent && typeof parsedContent.tool_execution === 'object') {
    ...
    if (args.attachments) {
      attachments = normalizeWorkspaceAttachmentPaths(args.attachments);
    }
...
const extractFromLegacyFormat = (content: any): { 
  ...
  if (toolData.toolResult && toolData.arguments) {
    ...
    if (toolData.arguments.attachments) {
      attachments = normalizeWorkspaceAttachmentPaths(toolData.arguments.attachments);
    }
  ...
  if (attachmentsMatch) {
    attachments = normalizeWorkspaceAttachmentPaths(attachmentsMatch[1]);
  }
