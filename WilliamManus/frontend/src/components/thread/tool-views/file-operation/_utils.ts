...
import { normalizeWriteFileArgs } from '@/lib/write-file-stream';
...
export const getOperationType = (name?: string, assistantContent?: any): FileOperation => {
  if (name) {
    if (name.includes('create') || name.includes('write')) return 'create'; // Include write for sandbox write_file
...
/**
 * Extract file content from incomplete/partial JSON string during streaming.
 * Handles JSON escape sequences (\n, \t, \\, \") in the extracted content.
 * This function is used to display file content in real-time as it streams from the LLM.
 */
export function extractPartialFileContent(
  streamingText: string,
  operation: FileOperation
): string | null {
  if (!streamingText) return null;

  const normalizedWriteFileArgs = normalizeWriteFileArgs(streamingText);
  if (normalizedWriteFileArgs) {
    if (typeof normalizedWriteFileArgs.file_contents === 'string') {
      return normalizedWriteFileArgs.file_contents;
    }
    if (typeof normalizedWriteFileArgs.file_contents_delta === 'string') {
      return normalizedWriteFileArgs.file_contents_delta;
    }
  }

  // Support multiple parameter names: file_contents, content, code_edit
  const keyPatterns = ['file_contents', 'content', 'file_contents_delta', 'code_edit'];

  for (const keyName of keyPatterns) {
    const pattern = new RegExp(`"${keyName}"\\s*:\\s*"`);
    const startMatch = streamingText.match(pattern);

    if (startMatch && startMatch.index !== undefined) {
      const startIndex = startMatch.index + startMatch[0].length;
      let rawContent = streamingText.substring(startIndex);

      // Find closing quote (excluding escaped quotes)
      // Use a simple approach: find the first unescaped quote
      let endIndex = -1;
      let i = 0;
