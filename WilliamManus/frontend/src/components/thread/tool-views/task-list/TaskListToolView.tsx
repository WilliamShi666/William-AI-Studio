...
const TaskItem: React.FC<{ task: Task; index: number }> = ({ task, index }) => {
  ...
  return (
    <div className="flex items-center gap-3 py-3 px-4 transition-colors border-b border-border/50 last:border-b-0 tool-panel-item">
      {/* Status Icon */}
...
const SectionHeader: React.FC<{ section: Section }> = ({ section }) => {
  ...
  return (
    <div className="flex items-center justify-between py-3 px-4 tool-panel-surface-subtle border-b border-border/60">
      <h3 className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{section.title}</h3>
      <div className="flex items-center gap-2">
        <Badge variant="outline" className="text-xs h-5 px-2 py-0 font-normal tool-panel-surface-subtle border-border/60">
...
export const TaskListToolView: React.FC<ToolViewProps> = ({
  ...
  return (
    <Card className="gap-0 flex border shadow-none border-t border-b-0 border-x-0 p-0 rounded-none flex-col h-full overflow-hidden bg-card">
      <CardHeader className="h-14 tool-panel-surface-subtle backdrop-blur-sm border-b border-border/60 p-2 px-4 space-y-2">
