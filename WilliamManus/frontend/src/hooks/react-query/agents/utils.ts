...
export const getAgents = async (params: AgentsParams = {}): Promise<AgentsResponse> => {
  try {
    ...
    if (!agentPlaygroundEnabled) {
      // When custom agents are disabled, return an empty list instead of throwing
      return {
        agents: [],
        pagination: {
          total: 0,
...
export const getThreadAgent = async (threadId: string): Promise<ThreadAgentResponse> => {
  try {
    const agentPlaygroundEnabled = await isFlagEnabled('custom_agents');
    if (!agentPlaygroundEnabled) {
      // Custom agents disabled: return a neutral response so chat can proceed
      return {
        agent: null,
        source: 'none',
        message: 'Custom agents disabled',
      };
    }
    const supabase = createClient();
    const { data: { session } } = await supabase.auth.getSession();
