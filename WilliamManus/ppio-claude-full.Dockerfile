...
# This full template intentionally extends the complete agent-skills-v12 image
# instead of the fast Claude-only template, so it retains Manim, Remotion,
# Netlify, frontend design, data-science, and document-processing capabilities.
...
# Claude Agent Service used by SandboxClaudeBridge.
COPY backend/agentscope_integration/claude_agent_service.py /opt/claude_agent_service.py

# Make project skills discoverable by Claude Code for root and non-root runs.
# The bridge also fixes /workspace ownership at runtime before launching Claude.
RUN mkdir -p /root/.claude /home/user/.claude /workspace/.claude && \
    ln -sfn /opt/claude_skills /root/.claude/skills && \
    ln -sfn /opt/claude_skills /home/user/.claude/skills && \
    ln -sfn /opt/claude_skills /workspace/.claude/skills && \
    chmod -R 777 /home/user/.claude /workspace/.claude

# Verify retained full-template capabilities plus Claude SDK capabilities.
RUN node --version && npm --version && \
    python3 -c "import numpy, pandas, manim, xgboost, transformers; print('full data/viz deps: OK')" && \
    python3 -c "import fastapi, flask, uvicorn, starlette; print('web deps: OK')" && \
    python3 -c "from weasyprint import HTML; from xhtml2pdf import pisa; from fpdf import FPDF; import markdown, tabulate; print('doc deps: OK')" && \
    python3 -c "import pypdf, pdfplumber, reportlab, openpyxl, pptx, pypdfium2; print('claude skills deps: OK')" && \
    summarize --help >/dev/null && echo "summarize CLI: OK" && \
    node -e "require('pdf-lib'); console.log('pdf-lib: OK')" && \
    node -e "console.log('remotion:', require('@remotion/cli/package.json').version)" && \
    ...
    ls /opt/claude_skills/pdf-ultimate/SKILL.md && \
    ls /opt/claude_skills/docx-ultimate/SKILL.md && \
    ls /opt/claude_skills/pptx-ultimate/SKILL.md && \
    ls /opt/claude_skills/xlsx-ultimate/SKILL.md && \
    ls /home/user/.claude/skills && \
    echo "Full Claude Agent template ready"
