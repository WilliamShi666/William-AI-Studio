...
Claude Code 将 Claude Code CLI 的全部能力封装为友好的 Web 界面，支持实时对话流、工具调用可视化、权限确认（HIL）、会话管理、MCP 扩展、终端集成等功能，适合个人开发者、团队协作以及 AI 编程教学场景。
...
```bash
git clone <repo-url> claude-code-ui-src
cd claude-code-ui-src
pnpm install
```
...
启动后，浏览器打开 http://localhost:5174，若 Claude Code CLI 未认证，会自动跳转到 **Settings 向导**：
...
Claude Code UI is a local developer control plane. By default:

- the backend listens on `127.0.0.1:3001`;
- the Vite frontend listens on `127.0.0.1:5174`;
- `/api/*` and `/ws/*` are expected to be reachable only from the same machine.

Do not set `HOST=0.0.0.0`, a LAN address, or a public interface unless you have placed the app behind your own authentication, network isolation, and TLS controls. The server prints a warning when `HOST` is set to a non-loopback address.

High-risk capabilities exposed by the local API:

| Surface | Examples | Risk if exposed beyond localhost |
|---------|----------|----------------------------------|
| File APIs | `/api/files/*`, `/api/attachments/*` | Browse, read, create, rename, delete, or upload files under developer-selected paths. |
| Terminal WebSocket | `/ws/terminal` | Interactive shell access through the local `node-pty` bridge. |
| Claude/session controls | `/ws/chat`, `/api/sessions/*`, `/api/system/claude-*` | Send prompts, inspect sessions, rollback/rewind checkpoints, run Claude diagnostics or updates. |
| Local configuration | `/api/config`, `/api/system/proxy`, `/api/system/claude-settings`, `/api/mcp/*`, `/api/hooks/*` | Read or write proxy settings, Claude environment settings, MCP servers, hooks, memory, skills, plugins, agents, and workflows. |

Low-risk health/status endpoints such as `/api/health` are still served by the same local process and should be treated as part of the same localhost-only service.
...
```bash
pnpm dev        # 开发模式（前端 :5174 + 后端 :3001 热重载）
pnpm build      # 生产构建
