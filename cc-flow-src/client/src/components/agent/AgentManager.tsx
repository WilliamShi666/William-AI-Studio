      ...
      <div className="flex flex-wrap gap-1 mb-1">
        {value.map((tag) => (
          <span key={tag} className="inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.5
            bg-[color:color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-[color:var(--app-accent-hover)] border border-[color:color-mix(in_srgb,var(--app-accent)_20%,transparent)] rounded">
            {tag}
        ...
        {agents.map((a) => (
          <div key={a.name}
            ...
            <div className="flex items-center gap-3 pl-5 flex-wrap">
              {a.background && (
                <span className="inline-flex items-center gap-0.5 text-[9px] text-[color:var(--app-accent-hover)]">
                  <Clock size={8} /> 后台
                </span>
              )}
              {a.isolation && (
