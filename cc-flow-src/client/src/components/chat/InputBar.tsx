...
const RUNTIME_EFFORT_VALUES = new Set(["low", "medium", "high", "max"]);
...
function resolveRuntimeConfig(
  ...
  const envEffort = env.CLAUDE_CODE_EFFORT_LEVEL?.trim();

  return {
    model: envModel || fallbackModel,
    effort: envEffort && RUNTIME_EFFORT_VALUES.has(envEffort)
      ? envEffort
      : fallbackEffort,
  };
}
...
export default function InputBar() {
  ...
  const { claudeInfo, claudeSettingsEnv } = useSystemStore();
  const runtimeConfig = resolveRuntimeConfig(model, effort, claudeSettingsEnv);
  ...
  }, [text, isStreaming, runtimeConfig.model, runtimeConfig.effort, runMode, notInstalled, noProject, currentSessionId, pendingFork, attachments, apiKey]);
  ...
  const handleSlashSelect = useCallback((cmd: SlashCommand) => {
    ...
    if (cmd.type === "prompt" && cmd.promptText) {
      ...
      const { claudeSettingsEnv: env } = useSystemStore.getState();
      const slashRuntimeConfig = resolveRuntimeConfig(m, e, env);
      const { runMode: rm } = useUIStore.getState();
      const sid = useChatStore.getState().currentSessionId;
      wsService.send("send_message", {
        prompt,
        model: slashRuntimeConfig.model,
        effort: slashRuntimeConfig.effort,
        runMode: rm,
  ...
  return (
    <div className="flex-shrink-0 glass-panel border-t border-[color:var(--app-border)] p-4">
      <div className="relative max-w-3xl mx-auto">
        ...
        <div className="absolute left-4 top-3.5 text-[color:var(--app-text-subtle)] pointer-events-none" style={{ zIndex: 1 }}>
          <Sparkles size={18} />
        </div>
        ...
        <div className={`relative rounded-xl border transition-colors overflow-hidden ${
          notInstalled || noProject
            ...
            : "border-[color:var(--app-border)] focus-within:border-[color:var(--app-accent)]"
        }`}
          ...
          <textarea
            ...
            rows={1}
            className="w-full bg-transparent text-sm text-[color:var(--app-text)] placeholder-[color:var(--app-text-subtle)] pl-12 pr-14 py-3.5 resize-none focus:outline-none font-sans leading-relaxed disabled:opacity-50"
          ...
          <div className="absolute right-3 top-2.5">
            {isStreaming ? (
              ...
              <button
                onClick={handleSend}
                disabled={!text.trim()}
                className="p-1.5 rounded-lg text-[color:var(--app-text)] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                style={{ background: text.trim() ? "var(--app-accent)" : "var(--app-control)" }}
                title="发送 (Enter)"
              >
                <ArrowUp size={16} />
              </button>
        ...
        <div className="flex items-center justify-between mt-2 px-1">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-1.5 hover:bg-[color:var(--app-hover)] rounded-lg text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text)] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            ...
            />
            {/* Separator */}
            <div className="w-px h-4 bg-[color:var(--app-control)] mx-1" />
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border transition-all hover:opacity-90 disabled:opacity-30 disabled:cursor-not-allowed ${
                runMode === "plan"
                  ? "border-[color:var(--app-accent)] bg-[color:color-mix(in_srgb,var(--app-accent)_12%,transparent)] text-[color:var(--app-accent-hover)]"
                  : runMode === "edit"
                    ? "border-amber-glow/30 bg-amber-glow/10 text-amber-glow"
                    : "border-[color:var(--app-border)] bg-[color:var(--app-hover)] text-[color:var(--app-text-muted)]"
              }`}
              title="点击切换运行模式"
            >
          ...
          <div className="flex items-center gap-3">
            {notInstalled ? (
              ...
              <span className="text-[10px] text-[color:var(--app-text-subtle)] hidden sm:block">
              ...
              <>
                {pendingPermCount > 0 ? (
                  ...
                  <span className={`text-[11px] flex items-center gap-1.5 ${
                    isStreaming ? "text-emerald-ok/80 animate-pulse" : "text-[color:var(--app-text-muted)]"
                  }`}>
                ...
                <span className="text-[10px] text-[color:var(--app-text-subtle)] font-mono">
                  {runtimeConfig.model} / {runtimeConfig.effort}
                </span>
              </>
            )}
          </div>
