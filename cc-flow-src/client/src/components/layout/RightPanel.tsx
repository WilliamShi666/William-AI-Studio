...
export default function RightPanel() {
  ...
  if (!rightPanelOpen) {
    return (
      <button
        ...
        className="w-7 flex-shrink-0 flex items-center justify-center border-l border-[color:var(--app-border)]
                   text-[color:var(--app-text-subtle)] hover:text-amber-glow transition-all z-10 group relative"
  ...
  return (
    <aside
      className="flex-shrink-0 relative overflow-hidden panel-transition z-10 right-panel-contrast"
      style={{
    ...
    >
      ...
      <div
        className="absolute inset-y-0 left-0 w-[5px] cursor-ew-resize z-20 hover:bg-[color:var(--app-hover)] transition-colors"
        onMouseDown={handleMouseDown}
      />

      {/* ── Main content with left border ── */}
      <div className="flex flex-col h-full border-l border-[color:var(--app-border)]">

        {/* ── Tab bar (hidden when terminal is maximized) ── */}
        {!termMaximized && (
          <div className="flex items-center gap-0 px-3 border-b border-[color:var(--app-border)] flex-shrink-0 pt-1">
            ...
            <button
              ...
              className="ml-auto p-1.5 rounded-md hover:bg-amber-glow/10 text-[color:var(--app-text-muted)] hover:text-amber-glow transition-all"
              title="收起右侧栏"
            >
...
function LiveMonitorPanel() {
  ...
  if (!isStreaming && allToolCalls.length === 0 && !lastTaskResult) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-6">
        <div className="w-12 h-12 rounded-2xl bg-[color:var(--app-hover)] border border-[color:var(--app-border)] flex items-center justify-center">
          <Activity size={20} className="text-[color:var(--app-text-subtle)]" />
        </div>
        <div>
          <div className="text-sm font-medium text-[color:var(--app-text-muted)] mb-1">无活跃任务</div>
          <div className="text-xs text-[color:var(--app-text-subtle)] leading-relaxed">
  ...
  return (
    <div className="flex-1 overflow-y-auto min-h-0 flex flex-col">
      ...
      {currentGoal && (
        <div className="p-4 border-b border-[color:var(--app-border)] flex-shrink-0" style={{ background: "var(--app-bg-soft)" }}>
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-[10px] font-bold text-[color:var(--app-text-muted)] uppercase tracking-wider">Current Goal</span>
            <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 text-[10px] font-bold border border-blue-500/20">
              IN PROGRESS
            </span>
          </div>
          <p className="text-sm font-medium text-[color:var(--app-text)] leading-snug line-clamp-2">{currentGoal}</p>

          {/* Elapsed + progress indicator */}
          <div className="flex items-center justify-between mt-2.5 mb-1">
            <div className="flex items-center gap-1.5 text-[10px] text-[color:var(--app-text-subtle)]">
              <Clock size={9} />
            ...
            {currentPlanState.hasPlan ? (
              ...
              <span className="flex items-center gap-1 text-[10px] text-[color:var(--app-text-subtle)]">
                <Wrench size={9} />
                {total} 工具调用
              </span>
            )}
          ...
          {currentPlanState.hasPlan && (
            <div className="h-1 w-full rounded-full overflow-hidden mb-3" style={{ background: "var(--app-control)" }}>
              <div
                className="h-full rounded-full transition-all duration-500"
                style={{
                  width: `${Math.max(currentPlanState.progressPct, 5)}%`,
                  background: "linear-gradient(90deg, var(--app-accent) 0%, var(--app-accent-hover) 100%)",
                }}
              />
            </div>
          )}
          ...
          {contextTokens > 0 && (
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] text-[color:var(--app-text-subtle)]">Context 用量</span>
                <span className="text-[10px] font-mono" style={{ color: ctxColor }}>
                  {fmtNum(contextTokens)} / {fmtNum(contextMax)} ({ctxPct}%)
                </span>
              </div>
              <div className="h-1 w-full rounded-full overflow-hidden" style={{ background: "var(--app-control)" }}>
                <div
      ...
      {(currentPlanState.hasPlan || isPlanning) && (
        <div className="p-3 border-b border-[color:var(--app-border)] flex-shrink-0">
          <div className="flex items-center gap-1.5 mb-2">
            <ListChecks size={12} className="text-[color:var(--app-accent)]" />
            <span className="text-[10px] font-bold text-[color:var(--app-text-muted)] uppercase tracking-wider">执行计划</span>
          </div>
          {isPlanning && !currentPlanState.hasPlan ? (
            <div className="flex items-center gap-2 py-2">
              <div className="h-2 flex-1 rounded-full overflow-hidden bg-[color:var(--app-hover)]">
                <div className="h-full w-1/3 rounded-full bg-[color:color-mix(in_srgb,var(--app-accent)_40%,transparent)] animate-pulse" />
              </div>
              <span className="text-[10px] text-[color:var(--app-text-subtle)]">正在规划...</span>
            </div>
          ) : (
...
function IdleSummaryPanel({
  ...
  if (!seg) {
    return (
      <div className="flex-1 flex items-center justify-center text-xs text-[color:var(--app-text-subtle)]">无任务记录</div>
    );
  }
    ...
    <div className="flex-1 overflow-y-auto min-h-0 flex flex-col">
      ...
      <div className="p-4 border-b border-[color:var(--app-border)] flex-shrink-0" style={{ background: "var(--app-bg-soft)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] font-bold text-[color:var(--app-text-muted)] uppercase tracking-wider flex-1">
            任务摘要
          </span>
          {/* Carousel controls */}
          {count > 1 && (
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={!hasPrev}
                className="p-0.5 rounded hover:bg-[color:var(--app-control)] text-[color:var(--app-text-muted)] disabled:opacity-20 transition-colors"
              >
                <ChevronLeft size={12} />
              </button>
              <span className="text-[10px] font-mono text-[color:var(--app-text-subtle)] min-w-[3ch] text-center">
                {viewIndex + 1}/{count}
              </span>
              <button
        ...
        {/* Goal */}
        {seg.goal && (
          <p className="text-xs text-[color:var(--app-text-muted)] mb-3 leading-snug line-clamp-2">{seg.goal}</p>
        )}

        {/* Plan checklist (compact, if present) */}
        {seg.planState.hasPlan && (
          <div className="mb-3 p-2 rounded-lg border border-[color:var(--app-border)]" style={{ background: "var(--app-control)" }}>
            <div className="flex items-center gap-1.5 mb-1.5">
              <ListChecks size={10} className="text-[color:var(--app-accent)]" />
              <span className="text-[10px] text-[color:var(--app-text-subtle)]">
...
function SummaryItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-1.5 p-2 rounded-lg border border-[color:var(--app-border)]" style={{ background: "var(--app-control)" }}>
      <span className="text-[color:var(--app-text-subtle)]">{icon}</span>
      <span className="text-[10px] text-[color:var(--app-text-subtle)]">{label}</span>
      <span className="text-[10px] font-mono text-[color:var(--app-text)] ml-auto">{value}</span>
    </div>
  );
}
        ...
        <div key={t.id} className={`flex items-start gap-2 ${compact ? "py-0.5" : "py-1"} ${
          ...
          <div className="flex-shrink-0 mt-0.5">
            {t.status === "completed" ? (
              ...
              <div className={`${compact ? "w-[11px] h-[11px]" : "w-[13px] h-[13px]"} rounded-full border border-[color:var(--app-border)]`} />
            )}
          </div>
          {/* Text */}
          <div className="flex-1 min-w-0">
            <span className={`${compact ? "text-[10px]" : "text-xs"} leading-snug ${
              t.status === "completed" ? "text-[color:var(--app-text-subtle)] line-through decoration-[color:var(--app-text-subtle)]" :
              t.status === "in_progress" ? "text-[color:var(--app-text)]" :
  ...
  return (
    <div className={`flex gap-3 items-start relative ${!isDone && !isRunning && !isError ? "opacity-50" : ""}`}>
      ...
      <div className={`flex flex-col gap-0.5 flex-1 min-w-0 ${
        isRunning
          ? "p-2.5 rounded-lg border border-[color:var(--app-border)] relative overflow-hidden"
          : ""
      }`} style={isRunning ? { background: "var(--app-hover)" } : {}}>
        ...
        <div className="flex items-center justify-between gap-2">
          <span className={`text-sm font-mono truncate ${
            isDone ? "text-[color:var(--app-text-muted)] line-through decoration-[color:var(--app-text-subtle)]" :
            isError ? "text-rose-err" :
            "text-[color:var(--app-text)] font-medium"
          }`}>
            {name}
          </span>
          {isRunning && (
        ...
        {detail && (
          <span className="text-[10px] text-[color:var(--app-text-subtle)] font-mono truncate">
            {detail.slice(0, 60)}
          </span>
        )}
      </div>
...
function ExtensionsPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex gap-0 border-b border-[color:var(--app-border)] flex-shrink-0 px-2">
...
function AgentPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Sub-tab bar */}
      <div className="flex gap-0 border-b border-[color:var(--app-border)] flex-shrink-0 px-3">
...
function EmbeddedTerminal({
  ...
  if (!terminalOpen) {
    return (
      <div
        className="flex-shrink-0 border-t border-[color:var(--app-border)] flex items-center gap-2 px-3 py-1.5 cursor-pointer hover:bg-[color:var(--app-hover)] transition-colors select-none"
        onClick={toggleTerminal}
        title="展开终端"
      >
        <TerminalSquare size={13} className="text-[color:var(--app-text-muted)] flex-shrink-0" />
        <span className="text-xs text-[color:var(--app-text-muted)] flex-1">终端</span>
        <ChevronUp size={13} className="text-[color:var(--app-text-muted)]" />
      </div>
    );
  }
  ...
  return (
    <div
      className={`flex flex-col border-t border-[color:var(--app-border)] ${
    ...
    >
      ...
      <div
        className="flex items-center border-b border-[color:var(--app-border)] flex-shrink-0 px-2 py-1 gap-1"
        style={{ background: "rgba(255,255,255,0.03)" }}
      >
        {/* Terminal icon — click to collapse */}
        <button
          onClick={toggleTerminal}
          className="p-1 rounded hover:bg-[color:var(--app-hover)] text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text-muted)] transition-colors flex-shrink-0"
          title="收起终端"
        >
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`group flex items-center gap-1.5 px-2 py-1 text-xs rounded whitespace-nowrap transition-colors ${
                tab.id === activeTab
                  ? "text-amber-glow bg-[color:var(--app-bg-deep)]"
                  : "text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)]"
              }`}
            >
              {tab.label}
              <X
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          <button
            ...
            className="p-1 rounded hover:bg-[color:var(--app-hover)] text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text-muted)] transition-colors disabled:opacity-40"
            title="新建终端"
          >
            <Plus size={12} />
          </button>
          <button
            ...
            className="p-1 rounded hover:bg-[color:var(--app-hover)] text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text-muted)] transition-colors"
      ...
      <div className="flex-1 min-h-0 overflow-hidden" style={{ position: "relative" }}>
        {activeTab ? (
          ...
          <div className="flex items-center justify-center h-full text-xs text-[color:var(--app-text-subtle)]">
            无活跃终端
          </div>
        )}
      </div>
