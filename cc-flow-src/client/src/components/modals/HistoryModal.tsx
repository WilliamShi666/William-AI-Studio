      ...
      <div className="modal-content glass-panel rounded-2xl w-full max-w-lg flex flex-col shadow-2xl shadow-black/50 max-h-[80vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-[color:var(--app-text)] font-display">历史会话</h2>
            {sessions.length > 0 && (
              <span className="text-[10px] text-[color:var(--app-text-muted)] bg-white/5 px-1.5 py-0.5 rounded-full">
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-white/5 text-[color:var(--app-text-muted)] hover:text-[color:var(--app-text)] transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-4 py-3 border-b border-white/5 flex-shrink-0 space-y-2">
          ...
          {sessions.length > 3 && (
            <div className="relative">
              <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[color:var(--app-text-subtle)]" />
              <input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="搜索会话名称、摘要、分支..."
                className="w-full text-xs bg-white/5 border border-white/8 rounded-lg pl-8 pr-3 py-1.5 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-[color:color-mix(in_srgb,var(--app-accent)_30%,transparent)] transition-colors"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery("")}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text-muted)]"
                >
                  <X size={12} />
                </button>
              )}
        ...
        <div className="flex-1 overflow-y-auto min-h-0 py-2">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              <Loader2 size={22} className="text-[color:var(--app-accent-hover)] animate-spin mb-3" />
              <p className="text-xs text-[color:var(--app-text-subtle)]">加载历史会话...</p>
            </div>
          ) : fetchError ? (
            <div className="flex flex-col items-center justify-center py-10 text-center px-5">
              <AlertCircle size={26} className="text-rose-err mb-3" />
              <p className="text-sm text-[color:var(--app-text)] mb-1.5">加载失败</p>
              <p className="text-[11px] text-[color:var(--app-text-subtle)] mb-4 font-mono break-all">{fetchError}</p>
              <button
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center px-4">
              <MessageSquare size={28} className="text-[color:var(--app-text-subtle)] mb-3" />
              <p className="text-sm text-[color:var(--app-text-muted)] mb-1">暂无历史会话</p>
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center px-4">
              <Search size={24} className="text-[color:var(--app-text-subtle)] mb-3" />
              <p className="text-sm text-[color:var(--app-text-muted)] mb-1">未找到匹配的会话</p>
              <p className="text-xs text-[color:var(--app-text-subtle)]">
                      ...
                      <div className="flex-1 min-w-0">
                        {editingId === s.id ? (
                          <input
                            ...
                            className="w-full text-sm font-medium text-[color:var(--app-text)] bg-white/5 border border-amber-glow/30 rounded px-1.5 py-0.5 outline-none focus:border-amber-glow/50"
                          />
                        ) : (
                          <div className="text-sm font-medium text-[color:var(--app-text)] truncate flex items-center gap-1.5">
                            {s.isPlanExecution && (
                              <span className="inline-flex items-center gap-0.5 text-[9px] font-medium px-1.5 py-0.5 rounded bg-[color:color-mix(in_srgb,var(--app-accent)_12%,transparent)] text-[color:var(--app-accent-hover)] flex-shrink-0">
                                <ListChecks size={9} />
                                Plan
                              </span>
                            )}
                            {s.name || <span className="text-[color:var(--app-text-subtle)] font-mono text-xs">{s.id.slice(0, 20)}…</span>}
                          </div>
                        )}
                        {/* Summary line (from sessions-index.json) */}
                        {s.summary && editingId !== s.id && (
                          <div className="text-[11px] text-[color:var(--app-text-muted)] truncate mt-0.5">
                            {s.summary}
                          </div>
                        )}
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          ...
                          {s.model && (
                            <span className="text-[10px] font-mono text-[color:var(--app-text-subtle)] bg-white/3 px-1.5 py-0.5 rounded">
                              {s.model}
                            </span>
                          )}
                          {s.gitBranch && (
                            <span className="inline-flex items-center gap-0.5 text-[10px] font-mono text-[color:color-mix(in_srgb,var(--app-accent)_70%,transparent)] bg-[color:color-mix(in_srgb,var(--app-accent)_8%,transparent)] px-1.5 py-0.5 rounded">
                              <GitBranch size={9} />
                              {s.gitBranch}
                            </span>
                          )}
                          {s.messageCount > 0 && (
                            <span className="inline-flex items-center gap-0.5 text-[10px] text-[color:var(--app-text-subtle)]">
