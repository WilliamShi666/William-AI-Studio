...
const MODELS: { id: ModelId; label: string; desc: string }[] = [
  { id: "opus",   label: "Claude Opus 4.7",   desc: "最强大，适合复杂任务" },
...
function StepIndicator({
  ...
  if (locked) {
    return (
      <div className="w-7 h-7 rounded-full border border-[color:var(--app-border)] bg-[color:var(--app-control)] flex items-center justify-center flex-shrink-0">
        <Lock size={12} className="text-[color:var(--app-text-subtle)]" />
      </div>
    );
  }
  if (done) {
...
function SectionTitle({
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      <span className="text-xs font-semibold text-[color:var(--app-text-muted)] uppercase tracking-wider">
        {label}
      </span>
    </div>
  );
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-semibold text-[color:var(--app-text-muted)] uppercase tracking-wider">代理设置</span>
        {stepLabel && (
      ...
      <div className="space-y-2.5">
        {(["httpProxy", "httpsProxy", "socksProxy"] as const).map((key) => (
          <div key={key}>
            <label className="block text-xs text-[color:var(--app-text-muted)] mb-1">
              {key === "httpProxy" ? "HTTP Proxy" : key === "httpsProxy" ? "HTTPS Proxy" : "SOCKS Proxy"}
              {key === "socksProxy" && (
                <span className="ml-1.5 text-[color:var(--app-text-subtle)] font-normal">（可选）</span>
              )}
            </label>
            <input
              type="text"
              ...
              className="w-full rounded-lg border border-[color:var(--app-border)] bg-[color:var(--app-control)] px-3 py-2 text-xs text-[color:var(--app-text)] placeholder-[color:var(--app-text-subtle)] focus:outline-none focus:border-[color:color-mix(in_srgb,var(--app-accent)_40%,transparent)] transition-colors font-mono"
            />
          </div>
        ))}
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {[
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-md text-[11px] border border-[color:var(--app-border)] text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)] hover:text-[color:var(--app-text)] transition-colors"
            >
              {p.label}
            </button>
          ))}
        ...
        {proxyTestResult && (
          <div className={`flex flex-col gap-1 text-xs px-3 py-2 rounded-lg border ${
            ...
            {proxyTestResult.success && (
              <span className="text-[10px] text-[color:var(--app-text-subtle)] pl-4">
...
function Step1EnvCard({ done }: { done: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {done ? (
        <div className="flex items-center gap-3 p-3 rounded-xl border border-emerald-ok/20 bg-emerald-ok/5">
          ...
          <div className="flex-1">
            <span className="text-sm text-[color:var(--app-text)]">已安装 Claude Code</span>
            {claudeInfo?.version && (
              <span className="ml-2 text-xs font-mono text-[color:var(--app-text-subtle)]">v{claudeInfo.version}</span>
            )}
          </div>
          <button
            onClick={loadClaudeInfo}
            disabled={infoLoading}
            className="p-1.5 rounded-lg hover:bg-[color:var(--app-hover)] text-[color:var(--app-text-muted)] hover:text-[color:var(--app-text)] transition-colors disabled:opacity-40"
            title="重新检测"
          >
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl border border-rose-err/20 bg-rose-err/5">
          ...
          <span className="text-sm text-[color:var(--app-text-muted)] flex-1">未检测到 Claude Code</span>
          <button
            onClick={loadClaudeInfo}
            disabled={infoLoading}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border border-[color:var(--app-border)] text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)] transition-colors disabled:opacity-40"
          >
      ...
      {!done ? (
        <section>
          <SectionTitle icon={Wrench} label="安装 Claude Code" color="text-[color:var(--app-accent-hover)]" />
          <div className="flex items-center gap-1.5 mb-2">
            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-[color:color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-[color:var(--app-accent-hover)] border border-[color:color-mix(in_srgb,var(--app-accent)_20%,transparent)]">
              第 2 步
            </span>
          </div>
          <div className="space-y-3">
            {isWindows && (
              <div className="p-3 rounded-xl border border-[color:var(--app-border)] bg-[color:var(--app-bg-soft)]">
                <div className="text-xs font-semibold text-[color:var(--app-text-muted)] mb-2">前置条件：Git for Windows</div>
                <a
                  href="https://git-scm.com/download/win"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border border-[color:var(--app-border)] text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)] transition-colors"
                >
                  下载 Git for Windows ↗
                </a>
                ...
                {(["powershell", "winget", "cmd"] as const).map((m) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-xs font-medium border transition-all ${
                      installMethod === m
                        ? "border-[color:color-mix(in_srgb,var(--app-accent)_30%,transparent)] bg-[color:color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-[color:var(--app-accent-hover)]"
                        : "border-[color:var(--app-border)] text-[color:var(--app-text-subtle)] hover:bg-[color:var(--app-hover)]"
                    }`}
                  >
            ...
            <div className="rounded-lg border border-[color:var(--app-border)] bg-[color:var(--app-control)] p-3">
              <pre className="text-[11px] text-[color:var(--app-text-muted)] font-mono whitespace-pre-wrap break-all leading-relaxed">
                {getInstallCmd()}
              </pre>
            </div>

            <div className="p-3 rounded-xl border border-amber-glow/20 bg-amber-glow/5 text-[11px] text-[color:var(--app-text-muted)] leading-relaxed">
            ...
            <div className="flex gap-2">
              <button
                onClick={handleCopy}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-[color:var(--app-border)] text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)] transition-colors"
              >
                <Copy size={12} />
                {copiedCmd ? "已复制!" : "复制命令"}
              </button>
        ...
        <section>
          <SectionTitle icon={Wrench} label="工具" color="text-[color:var(--app-text-muted)]" />
          <div className="space-y-3">
            {/* Update channel */}
            <div className="flex items-center gap-3">
              <span className="text-xs text-[color:var(--app-text-muted)] flex-shrink-0">更新频道</span>
              <div className="flex gap-1">
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-[color:var(--app-border)] text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)] transition-colors disabled:opacity-40"
              >
            ...
            {showUpdate && (
              <div className="rounded-lg border border-[color:var(--app-border)] p-3 bg-[color:var(--app-control)] max-h-28 overflow-y-auto">
                {updateLoading
                  ? <p className="text-xs text-[color:var(--app-text-subtle)] flex items-center gap-2"><Loader2 size={11} className="animate-spin" />正在更新...</p>
                  : <pre className="text-[11px] text-[color:var(--app-text-muted)] font-mono whitespace-pre-wrap">{updateOutput || "完成"}</pre>
                }
              </div>
            )}

            {showDoctor && (
              <div className="rounded-lg border border-[color:var(--app-border)] p-3 bg-[color:var(--app-control)] max-h-48 overflow-y-auto space-y-0.5">
                {doctorLoading
                  ? <p className="text-xs text-[color:var(--app-text-subtle)] flex items-center gap-2"><Loader2 size={11} className="animate-spin" />运行中...</p>
                  : doctorResult?.map((item, i) => (
                    <div key={i} className="flex items-start gap-2">
                      ...
                      {item.status === "info" && <Info size={11} className="text-[color:var(--app-text-subtle)] mt-0.5 flex-shrink-0" />}
    ...
    <div className="space-y-5">
      ...
      <section>
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-xs font-medium border transition-all ${
                authMode === m
                  ...
                  : "border-[color:var(--app-border)] text-[color:var(--app-text-muted)] hover:bg-[color:var(--app-hover)]"
              }`}
            >
        ...
        {authMode === "apikey" ? (
          <div className="space-y-2">
            <div className="relative">
              <input
                ...
                className="w-full rounded-lg border border-[color:var(--app-border)] bg-[color:var(--app-control)] px-3 py-2 pr-10 text-xs text-[color:var(--app-text)] placeholder-[color:var(--app-text-subtle)] focus:outline-none focus:border-emerald-ok/40 transition-colors font-mono"
              />
              <button
                type="button"
                onClick={() => setShowKey(!showKey)}
                className="absolute right-2.5 top-2 text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text-muted)] transition-colors"
              >
                {showKey ? <EyeOff size={14} /> : <Eye size={14} />}
              </button>
            </div>
            <p className="text-[11px] text-[color:var(--app-text-subtle)] leading-relaxed">
              仅存内存，刷新页面后需重新填写。去{" "}
              <a
          ...
          <div className="p-3 rounded-xl border border-[color:color-mix(in_srgb,var(--app-accent)_20%,transparent)] bg-[color:color-mix(in_srgb,var(--app-accent)_5%,transparent)] space-y-2">
            <p className="text-xs text-[color:var(--app-text-muted)] leading-relaxed">
              OAuth 授权需要在命令行中登录 Claude.ai 账号：
            </p>
            <ol className="text-[11px] text-[color:var(--app-text-muted)] list-decimal list-inside space-y-1 leading-relaxed">
              <li>点击页面底部"打开命令行"展开终端</li>
              <li>输入命令：<span className="font-mono text-[color:var(--app-accent-hover)]">claude</span></li>
      ...
      <section>
        ...
        <div className="p-3 rounded-lg border border-[color:var(--app-border)] bg-[color:var(--app-control)] font-mono text-[11px] text-[color:var(--app-text-muted)] space-y-1">
          {proxySettings.httpProxy
            ? <div>HTTP: <span className="text-[color:var(--app-text-muted)]">{proxySettings.httpProxy}</span></div>
            : <div className="text-[color:var(--app-text-subtle)]">HTTP: 未设置</div>
          }
          {proxySettings.httpsProxy
            ? <div>HTTPS: <span className="text-[color:var(--app-text-muted)]">{proxySettings.httpsProxy}</span></div>
            : <div className="text-[color:var(--app-text-subtle)]">HTTPS: 未设置</div>
          }
        </div>
      </section>
      ...
      {/* Model selection */}
      <section>
        <SectionTitle icon={Zap} label="模型" color="text-[color:var(--app-accent-hover)]" />
          ...
          {MODELS.map((m) => (
            <label
              ...
              className={`flex items-center gap-3 p-2.5 rounded-xl border cursor-pointer transition-all ${
                model === m.id
                  ? "border-[color:color-mix(in_srgb,var(--app-accent)_30%,transparent)] bg-[color:color-mix(in_srgb,var(--app-accent)_8%,transparent)]"
                  : "border-[color:var(--app-border)] hover:border-[color:var(--app-border)] hover:bg-[color:var(--app-hover)]"
              }`}
            >
              <input
                type="radio"
                ...
                className="accent-[color:var(--app-accent)]"
              />
              <div>
                <div className={`text-xs font-medium ${model === m.id ? "text-white" : "text-[color:var(--app-text)]"}`}>
                  {m.label}
                </div>
                <div className="text-[10px] text-[color:var(--app-text-subtle)]">{m.desc}</div>
              </div>
            </label>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between mt-3">
          <span className="text-xs text-[color:var(--app-text-muted)]">扩展思考 (Extended Thinking)</span>
          <button
            onClick={() => setThinking(!thinking)}
            className={`relative w-10 h-5 rounded-full transition-colors ${thinking ? "bg-[color:var(--app-accent)]" : "bg-[color:var(--app-control)]"}`}
          >
      ...
      {/* Test & Save */}
      <div className="pt-2 border-t border-[color:var(--app-border)] space-y-2">
        <button
...
function DomesticTab() {
  ...
  return (
    <div className="space-y-5">
      <div className="p-3 rounded-xl border border-[color:color-mix(in_srgb,var(--app-accent)_20%,transparent)] bg-[color:color-mix(in_srgb,var(--app-accent)_5%,transparent)] text-[11px] text-[color:var(--app-text-muted)] leading-relaxed">
        通过替换 Anthropic 兼容 API 端点，将 Claude Code 的基座模型替换为国产模型（如 Kimi、DeepSeek 等）。
        这些参数将写入 <span className="font-mono text-[color:var(--app-accent-hover)]">~/.claude/settings.json</span>。
      </div>

      <section>
        <SectionTitle icon={Globe} label="API 端点" color="text-[color:var(--app-accent-hover)]" />
        <input
          type="url"
          value={baseUrl}
          ...
          className="w-full rounded-lg border border-[color:var(--app-border)] bg-[color:var(--app-control)] px-3 py-2 text-xs text-[color:var(--app-text)] placeholder-[color:var(--app-text-subtle)] focus:outline-none focus:border-[color:color-mix(in_srgb,var(--app-accent)_40%,transparent)] transition-colors font-mono"
        />
        <p className="mt-1.5 text-[10px] text-[color:var(--app-text-subtle)]">
          示例：Kimi → <span className="font-mono">https://api.moonshot.ai/anthropic</span>
        </p>
      </section>

      <section>
        <SectionTitle icon={KeyRound} label="API Key" color="text-[color:var(--app-accent-hover)]" />
        <div className="relative">
          <input
            type={showKey ? "text" : "password"}
            value={apiKey}
            onChange={(e) => setApiKey(e.target.value)}
            placeholder="sk-你的密钥..."
            className="w-full rounded-lg border border-[color:var(--app-border)] bg-[color:var(--app-control)] px-3 py-2 pr-10 text-xs text-[color:var(--app-text)] placeholder-[color:var(--app-text-subtle)] focus:outline-none focus:border-[color:color-mix(in_srgb,var(--app-accent)_40%,transparent)] transition-colors font-mono"
          />
      ...
      <section>
        <SectionTitle icon={Server} label="模型名称" color="text-[color:var(--app-accent-hover)]" />
        <input
          type="text"
          value={model}
...
function Step2AuthCard({ locked }: { locked: boolean }) {
  ...
  return (
    <div className={`transition-opacity ${locked ? "opacity-40 pointer-events-none select-none" : ""}`}>
      {locked && (
        <div className="flex items-center gap-2 p-3 rounded-xl border border-[color:var(--app-border)] bg-[color:var(--app-bg-soft)] mb-4">
          <Lock size={14} className="text-[color:var(--app-text-subtle)]" />
          <span className="text-xs text-[color:var(--app-text-subtle)]">请先完成第①步（安装 Claude Code）</span>
        </div>
      )}
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
        <div className="absolute rounded-full blur-[120px]"
          style={{ top: "-10%", left: "-5%", width: "50%", height: "50%", background: "var(--app-glow)" }} />
        <div className="absolute rounded-full blur-[80px]"
          style={{ bottom: "-10%", right: "-5%", width: "35%", height: "40%", background: "var(--app-glow)" }} />
      </div>

      {/* ── Header ── */}
      <header className="relative z-10 flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-[color:var(--app-border)] glass-panel">
        <div className="flex items-center gap-2.5">
          <Settings size={15} className="text-amber-glow" />
          <h1 className="text-sm font-semibold text-[color:var(--app-text)] font-display">设置</h1>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 text-xs text-[color:var(--app-text-muted)]">
            ...
            {authStatus?.version && (
              <span className="font-mono text-[color:var(--app-text-subtle)]">v{authStatus.version}</span>
            )}
          </div>
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-[color:var(--app-text-subtle)] hover:text-[color:var(--app-text)] hover:bg-[color:var(--app-hover)] border border-[color:var(--app-border)] transition-colors"
            title="关闭 (Esc)"
          >
            <X size={14} />
          </button>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-6 py-7 space-y-5">
          ...
          {/* Terminal hint */}
          <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-[color:var(--app-border)] bg-[color:var(--app-bg-soft)]">
          ...
          <div className={`rounded-2xl border p-5 ${
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-sm font-semibold text-[color:var(--app-text)] font-display">
                  Claude Code 环境配置
                </h2>
                <p className="text-[11px] text-[color:var(--app-text-subtle)] mt-0.5">
          ...
          <div className={`rounded-2xl border p-5 ${
            !step1Done
              ...
              : step2Done
                ? "border-emerald-ok/15 bg-emerald-ok/[0.03]"
                : "border-[color:color-mix(in_srgb,var(--app-accent)_20%,transparent)] bg-[color:color-mix(in_srgb,var(--app-accent)_3%,transparent)]"
          }`}>
            <div className="flex items-center gap-3 mb-4">
              <StepIndicator done={step2Done} locked={!step1Done} number={2} />
              <div>
                <h2 className={`text-sm font-semibold font-display ${!step1Done ? "text-[color:var(--app-text-subtle)]" : "text-[color:var(--app-text)]"}`}>
                  授权 & 模型配置
                </h2>
                <p className="text-[11px] text-[color:var(--app-text-subtle)] mt-0.5">
                  {!step1Done
